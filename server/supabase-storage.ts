...
// IMPORTANT: These MUST come from environment variables (Replit Secrets)
...
if (!supabaseUrl || !supabaseServiceKey) {
  ...
  throw new Error('Missing Supabase configuration. Please set VITE_SUPABASE_URL and VITE_SUPABASE_SERVICE_ROLE_KEY in Replit Secrets.');
}

console.log('🔌 Connecting to Supabase:', supabaseUrl);
console.log('🔑 Service key loaded:', supabaseServiceKey ? `Yes (${supabaseServiceKey.substring(0, 20)}...)` : 'No');
...
console.log('📋 Env check:', {
  ...
  keyLength: process.env.VITE_SUPABASE_SERVICE_ROLE_KEY?.length
});
