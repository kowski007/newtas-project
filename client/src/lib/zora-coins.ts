...
export async function createCoinOnBaseSepolia(
  ...
  // Default to mainnet if no preference is set
  ...
  console.log(`✅ Using GASLESS deployment - Base Paymaster will sponsor all fees!`);

  try {
    // Skip ZORA balance check for gasless deployment
    // The Base Paymaster sponsors all gas fees when using smart accounts
    console.log("🎉 Gasless mode: Skipping ZORA balance check");
    console.log("💰 All fees will be sponsored by Base Paymaster");
